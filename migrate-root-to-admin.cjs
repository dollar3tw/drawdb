...
db.serialize(() => {
  ...
  db.run('BEGIN TRANSACTION', (err) => {
    ...
    // 步驟 1: 更新現有 root 用戶的角色為 admin
    db.run(`UPDATE users SET role = 'admin' WHERE role = 'root'`, function(err) {
      if (err) {
        console.error('❌ 更新用戶角色失敗:', err.message);
        db.run('ROLLBACK');
        return;
      }
      console.log(`✅ 已更新 ${this.changes} 個用戶的角色從 root 改為 admin`);
      
      // 步驟 2: 創建新的 users 表（包含新的約束）
      db.run(`CREATE TABLE IF NOT EXISTS users_new (
        ...
        db.run(`INSERT INTO users_new 
          SELECT * FROM users`, function(err) {
          if (err) {
            console.error('❌ 複製資料失敗:', err.message);
            db.run('ROLLBACK');
            return;
          }
          console.log(`✅ 已複製 ${this.changes} 筆資料到新表`);
